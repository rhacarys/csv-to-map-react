    ...
    fetch(config.API_FILES_ENDPOINT, {
      ...
      .then((result) => {
        if (result && result.data) {
          ...
          this.stoptLoading(
            "info",
            `${result.data.length} records found on history`
          );
        } else {
      ...
      .catch((error) =>
        this.stoptLoading("error", `An error has occurred: ${error.message}`)
      );
  };
    ...
    fetch(config.API_FILES_ENDPOINT, {
      ...
      .then((result) => {
        ...
        if (result && result.data && result.data.points) {
          ...
          this.stoptLoading(
            "success",
            "The data has been sent successfully and the markers updated!"
          );
        } else {
